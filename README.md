...
```
tanzu apps workload create simple-node-express -f config/workload.yaml \
  --local-path . \
  --source-image <REPOSITORY-PREFIX>/simple-node-express-source \
  --type web
```
...
```
tanzu apps workload get simple-node-express
```
